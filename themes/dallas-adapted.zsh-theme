# Personalized by Xiao from dallas!

# Grab the current date (%D) and time (%T) wrapped in {}: {MM/DD/YY HH:MM:SS}
DALLAS_CURRENT_TIME_="%{$fg[white]%}{%{$fg[yellow]%}%D{%m/%f/%y} %(2L.%L/.)%*%{$fg[white]%}}%{$reset_color%}"
